...
Widget textnumOfDetect(String num) {
  return Text(
    num,
    style: txtStyle(Colors.red, 35.0, true)
  );
}
//recommenditon
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        title:  Text('Share to' ,style: txtStyle(ColorManager.DarkBasiColor(context), 15.0, true),),
        content: Container(
...
Widget buildShareRow(String name, IconData icon, String image ,BuildContext context) {
  return Padding(
    ...
    child: Row(
      children: [
        ...
        Text(
          name,
          style:  TextStyle(color: ColorManager.DarkBasiColor(context), ),
        ),
        const Spacer(),
        IconButton(
          color: ColorManager.DarkBasiColor(context),
          icon: Icon(icon),
          onPressed: () {
            Navigator.pushReplacementNamed(context, RouteGenerator.Patient_QuestionScreen);
          },
        ),
      ],
    ),
